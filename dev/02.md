# nginx rewrite模块源码解读
nginx rewrite模块在配置解析阶段，通过解析rewrite指令的参数建立对应的语法函数列表，设置到location的codes数组上；在处理过程中ngx_http_rewrite_handler函数依次调用这些codes来解析对应的语句，拼接出rewrite后的目标URL，然后进行重定向。
```
/*
