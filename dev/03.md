# Nginx 基本数据结构
## 概述
nginx为了开发统一性和平台的可移植性，从而封装了大量的数据类型，本次只是简单介绍了 Nginx 对基本数据的一种封装，包括 基本整型数据类型、字符串数据类型、缓冲区类型以及 chain 数据类型，以便于后续开发。
...
/* 其中 intptr_t uintptr_t是ISO C99定义的，在文件 /usr/include/stdint.h 文件中*/
...
/*为了增加平台移植性，nginx对于数值型变量没有采用自由类型int等*/
```
### 字符串类型
```
/* Nginx 字符串数据类型 */
/* Nginx 字符串类型是对 C 语言字符串类型的简单封装，对str增加了len，避免计算字符串长度，也在一定程度上复用字符串，例如copy，具体还要看相关用例。
...
/*对于字符串，nginx采用宏定义，对其进行初始化或者赋值操作*/
...
/*
* str1 = ngx_string("hello world"); ==》 str1 = {sizeof("hello world")-1, (u_char *) "hello world"}
* struc不允许{}赋值
*/
/* 正确写法*/
ngx_str_t str1, str2;
